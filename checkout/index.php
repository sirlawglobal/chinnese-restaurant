...
function isUserLoggedIn() {
  // Replace with actual authentication check (e.g., check session or token via AJAX)
  // Example: return !!sessionStorage.getItem("user_token");
  return false; // Assume guest for now; implement actual logic
}

// Placeholder for getting logged-in user's email
function getUserEmail() {
  // Replace with actual logic to fetch user's email (e.g., from session or API)
  return "user@example.com"; // Fallback for logged-in users
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  proceedToPaymentBtn.addEventListener("click", async () => {
    ...
    if (!isUserLoggedIn() && !guestEmail.trim()) {
      alert("Please provide an email address for guest checkout.");
      return;
    }
    if (!isUserLoggedIn() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(guestEmail)) {
    ...
    cartItems.forEach((item, index) => {
      ...
      console.log(`Item ${index + 1}: ${item.name}, Price: ${price}, Quantity: ${quantity}, Item Total: ${itemTotal}`);
    });

    const deliveryFeeValue = cartItems.length > 0 ? 131.00 : 0;
    const taxesValue = cartItems.length > 0 ? 2.00 : 0;
    const discountValue = 0; // No discount
    const totalValue = itemTotalValue + deliveryFeeValue + taxesValue - discountValue;

    // Log breakdown
    console.log("Item Total:", itemTotalValue);
    console.log("Delivery Fee:", deliveryFeeValue);
    console.log("Taxes:", taxesValue);
    ...
    const orderData = {
      ...
      guest_email: isUserLoggedIn() ? null : guestEmail // Include guest email only for guests
    };
    ...
    try {
      ...
      FlutterwaveCheckout({
        ...
        customer: {
          email: isUserLoggedIn() ? getUserEmail() : guestEmail, // Use user email or guest email
          name: "Golden Dish Customer",
          phone_number: "1234567890" // Replace with actual phone number if available
        },
        customizations: {
