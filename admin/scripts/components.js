...
function getRelativePath(targetPath) {
  ...
  // Calculate depth (number of segments after base path)
  const depth = current.split('/').filter(segment => segment.length > 0).length;
  
  // Handle absolute paths differently
  if (targetPath.startsWith('/assets/') || targetPath.startsWith('/images/')) {
    // For root-level assets, return absolute path
    return targetPath;
  }
  
  // For admin paths, build relative path
  if (depth === 0) {
    return `.${targetPath}`;
  }
...
const sidebar = `
  <aside>
    <div class="user flex align-center justify-center">
      <div class="logo">
        <img src="${getRelativePath("assets/img/logo.webp")}" alt="Golden Dish">
      </div>
    </div>
    <nav>
                  ...
                  <div class="dropdown-content ${isActive || isSubItemActive ? "show" : ""}">
                    ${Object.entries(pageInfo.subItems)
                      .map(([subPath, subPageInfo]) => `
                        <a href="${getRelativePath(`${subPath}`)}"
...
function renderComponents() {
  fetch(getRelativePath(`assets/img/icons-sprite.svg`))
