...
createScheduleBtn.addEventListener("click", async (e) => {
  ...
  const newEvent = {
    user_id: 1, // Replace with dynamic ID if logged-in user system is implemented
  ...
  try {
    const response = await fetch("/chinnese-restaurant/admin/calendar/schedules.php", {
      method: "POST",
    ...
    if (result.status === "success") {
      alert("New schedule created and saved!");
      location.reload(); // reload to fetch from backend again
    } else {
      
      alert("Error saving schedule. Try again.");
    }
  } catch (err) {
    console.error("API Error:", err);
    alert("Server error. Please check your API.");
  }
});
...
function resetForm() {
  document.getElementById("scheduleForm")?.reset(); // optional chaining
  document.getElementById("scheduleId").value = "";
  document.getElementById("createScheduleBtn").textContent = "Create";
}
...
document.addEventListener("DOMContentLoaded", function () {
  console.log("DOM fully loaded 🚀"); // You MUST see this first

  fetchSchedulesFromServer(); // 🔥 this runs after page is ready
});
