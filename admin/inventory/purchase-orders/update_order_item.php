...
try {
    ...
    // Handle category
    $categoryId = null;
    if (is_numeric($item['category']) && in_array($item['category'], ['1', '2', '3'])) {
        $categoryId = intval($item['category']);
        error_log("Using provided category_id: $categoryId");
    } elseif ($item['category'] === '4' && !empty($item['category'])) {
        $categoryId = $item['category']; // Custom category name
        error_log("Using custom category: $categoryId");
    } else {
        // Fetch existing category_id
        $existingCatSql = "SELECT category_id FROM inves_order_items WHERE order_id = :order_id LIMIT 1";
        $existingCatResult = db_query($existingCatSql, [':order_id' => $orderId]);
        if ($existingCatResult && !empty($existingCatResult) && isset($existingCatResult[0]->category_id)) {
            $categoryId = $existingCatResult[0]->category_id;
            error_log("Retained existing category_id: $categoryId for order_id $orderId");
        } else {
            error_log("No valid category provided and no existing category found for order_id: $orderId");
            throw new Exception("Invalid or missing category");
        }
    }
    ...
    $orderParams = [
        ...
        ':status' => $status,
        ':delivery_date' => date('Y-m-d', strtotime('2025-06-12')), // June 12, 2025
        ':order_id' => $orderId
    ];
    ...
    error_log("Updated order ID: $orderId with status: $status");
    ...
    error_log("Item parameters for order ID $orderId: " . print_r($itemParams, true));
    $itemResult = db_query($itemSql, $itemParams);
    if ($itemResult === false) {
        error_log("Failed to update item for order_id $orderId: " . ($GLOBALS['DB_STATE']['error'] ?? 'No error'));
    ...
    error_log("Exception in update:order_id " . $e->getMessage());
    echo json_encode([
        'success' => false,
