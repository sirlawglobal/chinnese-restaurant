...
<html lang="en">
  ...
  <body class="flex">
    <style>
      ...
      .action-block { display: none; position: absolute; z-index: 2000; background-color: #ffeb3b; border: 1px solid #ccc; border-radius: 4px; padding: 5px; flex-direction: row; gap: 5px; }
      .action-block.show { display: flex; }
      ...
      .modal { display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 0; border-radius: 8px; z-index: 1001; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.15); max-height: 80vh; overflow-y: auto; width: 600px; max-width: 90%; }
      ...
      .item-row { display: flex; gap: 10px; margin-bottom: 15px; padding: 10px; border: 1px solid #eee; border-radius: 4px; align-items: flex-start; }
      .item-group { flex: 1; display: flex; gap: 10px; flex-wrap: wrap; }
      .remove-item { background-color: #dc3545; color: white; padding: 5px 10px; border: none; border-radius: 4px; cursor: pointer; align-self: center; margin-top: 20px !important; }
      .remove-item:hover { background-color: #c82333; }
      .category-custom { margin-top: 5px; }
      .modal-form-group { display: flex; flex-direction: column; min-width: 150px; }
      ...
      .category-select, .modal-form-input[type="text"], .modal-form-input[type="number"] { padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 14px; width: 100%; box-sizing: border-box; }
...
<select id="itemName_0" name="itemName_0" class="modal-form-input" required>
    ...
    <div class="modal" id="viewOrderModal">
      <div class="modal-content">
        ...
        <div class="view-modal-content">
          ...
          <div class="view-modal-field">
            <span class="view-modal-label">Item Name</span>
            <span class="view-modal-value" id="viewItemName"></span>
          </div>
          <div class="view-modal-field">
            <span class="view-modal-label">Category</span>
            <span class="view-modal-value" id="viewCategory"></span>
          </div>
          <div class="view-modal-field">
            <span class="view-modal-label">Vendor/Supplier</span>
          ...
          <div class="view-modal-field">
            <span class="view-modal-label">Delivery</span>
            <span class="view-modal-value view-modal-progress" id="viewDelivery"></span>
          </div>
          <div class="view-modal-field">
            <span class="view-modal-label">Unit Price</span>
            <span class="view-modal-value" id="viewUnitPrice"></span>
          </div>
          <div class="view-modal-field">
            <span class="view-modal-label">Quantity</span>
            <span class="view-modal-value" id="viewQuantity"></span>
          </div>
          <div class="view-modal-field">
    ...
    <script>
      $(document).ready(function () {
        ...
        var table = $("#inventory-table").DataTable({
          ...
          console.error('Delivery render error for order_id', data.order_id);
          return '<error>';
        }
      },
    },
      ...
      orderable: false
        ...
        $.ajax({
          ...
          success: function (response) {
            console.log('API Response:', JSON.stringify(response, null));
        ...
        $('.dropdown-menu a').on('click', function (e) {
          ...
          if (category === 'All Category') {
            ...
            table.column(2).search(category, false, true).draw();
          }
        ...
        $('#inventory-table').on('click', '.action-btn', function (e) {
          ...
          currentTrigger = null;
          if (action === 'view') {
            $('#viewOrderId').text(rowData.order_id || 'N/A');
            $('#viewItemName').text(rowData.item || 'N/A');
            $('#viewCategory').text(rowData.category || 'N/A');
            $('#viewVendor').text(rowData.vendor || 'N/A');
            $('#viewStatus').html(`<span class="status ${rowData.status.toLowerCase()}">${escapeHtml(rowData.status)}</span>`);
            var status = (rowData.status || 'pending').toLowerCase();
            var progress = status === 'delivered' ? 100 : status === 'shipped' ? 50 : 0;
            var date = formatWATDate(rowData.schedule_date);
            var statusText = status === 'delivered' ? 'Arrived' : status === 'shipped' ? 'In Transit' : 'Arrive';
            $('#viewDelivery').html(
              `<div class="view-modal-progress">` +
              `<div class="flex justify-between align-center">` +
              `<div class="view-modal-progress-bar-container">` +
              `<div class="view-modal-progress-bar ${status}" style="width: ${progress}%"></div>` +
              `</div>${progress}%</div>` +
              `<small>${escapeHtml(statusText)} ${date}</small></div>`
            );
            $('#viewUnitPrice').text('$' + parseFloat(rowData.unit_price || 0).toFixed(2));
            $('#viewQuantity').text(rowData.quantity || 'N/A');
            $('#viewTotalPrice').text('$' + parseFloat(rowData.total_price || 0).toFixed(2));
...
$('#addItemButton').on('click', function (e) {
  ...
  // Clone PHP-populated item select
  var $itemSelectClone = $('#itemName_0').clone();
  $itemSelectClone
    .attr('id', 'itemName_' + newIndex)
    .attr('name', 'itemName_' + newIndex)
    .val('');

  $itemGroup1.append(
    $('<div>').addClass('modal-form-group').append(
      $('<label>')
        .addClass('modal-form-label')
        .attr('for', 'itemName_' + newIndex)
        .text('Item Name'),
      $itemSelectClone
    ),
    $('<div>').addClass('modal-form-group').append(
      $('<label>')
        .addClass('modal-form-label')
        .attr('for', 'unitPrice_' + newIndex)
        .text('Unit Price ($)'),
      $('<input>')
        .addClass('modal-form-input')
        .attr({
          type: 'number',
          id: 'unitPrice_' + newIndex,
          name: 'unitPrice_' + newIndex,
          step: '0.01',
  ...
  // Clone PHP-populated category select
  var $categorySelectClone = $('#categorySelect_0').clone();
  $categorySelectClone
    .attr('id', 'categorySelect_' + newIndex)
    .attr('name', 'categorySelect_' + newIndex)
    .val('');

  // Custom category input
  var $customCategoryInput = $('<input>')
    .addClass('modal-form-input category-custom')
    .attr({
      type: 'text',
      id: 'categoryCustom_' + newIndex,
      name: 'categoryCustom_' + newIndex,
  ...
  $itemGroup2.append(
    $('<div>').addClass('modal-form-group').append(
      $('<label>')
        .addClass('modal-form-label')
        .attr('for', 'categorySelect_' + newIndex)
        .text('Category'),
      $categorySelectClone,
      $customCategoryInput
    ),
    ...
    $('<div>').addClass('modal-form-group').append(
      $('<label>')
        .addClass('modal-form-label')
        .attr('for', 'status_' + newIndex)
        .text('Status'),
      $('<select>')
        .addClass('modal-form-input status-select')
        .attr({
          id: 'status_' + newIndex,
          name: 'status_' + newIndex,
          required: true
        })
        .append(
          $('<option>').val('pending').text('Pending'),
  ...
  $('#categorySelect_' + newIndex).on('change', function () {
    if ($(this).val() === '4') {
      $('#categoryCustom_' + newIndex).show();
    } else {
      $('#categoryCustom_' + newIndex).hide();
    }
  });
